...
let yellow = false;
let hasPlayedFinalLap = false;
let sc = false;
let vsc = false;
let red = false;
let version = '';
let themeSelectReference: JQuery;
let miscOptionsReference: JQuery;
...
let oldMessages: RaceControlMessages = {
  Messages: [
    {
      Message: '',
      Category: 'Other',
  ...
  try {
    ...
    if (!countDownRunning) {
      const countdownTimer = setInterval(() => {
        if (timeleft <= 0) {
          void linkF1MV();
          getCurrentSessionInfo().catch((error: unknown) => {
            console.error('Error while getting Current Session Info:', error);
          });
    ...
    console.error(error);
    linkF1MV().catch((error: unknown) => {
      console.error('Error while Linking F1MV:', error);
    });
  }
}
/**
...
async function getCurrentSessionInfo(): Promise<string> {
  try {
    ...
    raceName = raceYear.toString() + ' ' + sessionName;
    $('#raceName').text(raceName + ' ' + sessionType);
    if (debugOn) console.log(`Current Race Name: ${raceName}`);
    return raceName;
  } catch (error) {
    if (debugOn) console.error('Unable to Get Data From MultiViewer GraphQL API:' + '\n' + (error as Error).message);
...
async function getPixooIPs(): Promise<PixooDeviceList[]> {
  try {
    ...
    const pixooData: PixooData = (await response.json()) as PixooData;

    // Extract all device IPs
    pixooIPs = pixooData.DeviceList.map((device) => device.DevicePrivateIP);

    // Send all IPs to the main process
    globalThis.api.getPixooIP(pixooIPs).catch((error: unknown) => {
      console.error('Error while sending Pixoo IP:', error);
    });
...
async function initializePixoo() {
  ...
  if (pixoostartup && currentMode === 1) {
    ...
    if (mvLogoSwitch) {
      changeGif('pixoostartupMV', currentMode).catch((error: unknown) => {
        console.error('Error while changing GIF:', error);
      });
    } else {
      changeGif('pixoostartup', currentMode).catch((error: unknown) => {
        console.error('Error while changing GIF:', error);
      });
    }
    //Wait for Pixoo Startup Gif to Finish
    await timer(10000);
    changeGif('void', currentMode).catch((error: unknown) => {
      console.error('Error while changing GIF:', error);
    });
    await timer(2000);
...
function getCurrentTrackPath(currentMapTheme: number): string {
  if (trackMapSwitch) {
    ...
    if (mapTheme && Array.isArray(mapTheme.seasons)) {
      for (let index = 0; index < mapTheme.seasons.length; index++) {
        const season = mapTheme.seasons[index];

        // Check if the season is defined and has trackMaps
        if (season) {
          const trackMaps = season.trackMaps;

          // Check if raceName is in trackMaps
          if (raceName in trackMaps) {
            const trackMapPath = trackMaps[raceName];
            if (trackMapPath) {
          ...
          if (debugOn) {
            console.log(`Season at index ${index} is undefined or has no trackMaps.`);
          }
        }
      }
    } else {
      if (debugOn) {
        console.log('mapTheme is undefined or seasons is not an array.');
      }
    }
    ...
    for (const element of driverNumbersArray) {
      ...
      if (DriverNumbers && DriverSeason) {
        ...
        if (raceYearRegex.test(DriverSeason)) {
          ...
          return DriverNumberPath ?? 'Driver Number Not Found'; // Handle potential undefined
        }
      }
    }
  }
...
function createNewInstance(url?: string | URL, windowTitle?: string) {
  if (arguments.length === 0) {
    void globalThis.api.openNewWindow();
  }
  try {
...
function getGifPath(flag: string, driverNumber?: string) {
  let flagPath: string | boolean | DriverNumbers[] | undefined;
  ...
  if (flag === 'void' && mvLogoSwitch) {
    for (const theme of themes) {
      if (theme.id === currentTheme) {
        flagPath = theme.gifs.mv;
      }
    }
  } else if (flag === 'void' && trackMapSwitch && trackMapPath.slice(((trackMapPath.lastIndexOf('.') - 1) >>> 0) + 2)) {
    ...
    for (const theme of themes) {
      if (theme.id === currentTheme) {
        flagPath = theme.gifs[flag as keyof Gifs];
      }
    }
    if (driverNumber) {
...
function selectTheme(id: number) {
  if (debugOn) log('Theme Selected: ' + id.toString());
  currentTheme = id;
    ...
    $('#trackMapSwitch').on('change', () => {
      if (trackMapSwitch) {
        ...
        for (const mapTheme of mapThemes) {
          $('#trackMapStyleSelect').append(`<option id=${mapTheme.id} value=${mapTheme.id}>${mapTheme.name}</option> `);
        }
        ...
        $('#trackMapStyleSelect').on('change', () => {
          ...
          if (mapID !== undefined) {
            selectMapTheme(Number.parseInt(mapID.toString()));
            $('#launchDigiFlag').prop('disabled', false);
          }
        });
        return trackMapSwitch;
...
$(function () {
  ...
  $('#openMultiViewer').on('click', () => {
    createNewInstance('https://muvi.gg/go/app');
  });
  ...
  if (result.Messages.length === oldMessages.Messages.length) {
    ...
    if (recentMessage) {
      ...
      if (/black and orange/i.test(recentMessage.Message)) {
        const carNumberMatch = /car (\d+)/i.exec(recentMessage.Message);
        if (carNumberMatch) {
          const carNumber = carNumberMatch[1];
          const theme = themes[currentTheme];
          changeGif('mec', currentMode).catch((error: unknown) => {
            console.error('Error while changing GIF:', error);
          });
          await timer(3500);
          if (carNumber) {
            if (carNumber in theme.gifs) {
              changeGif(carNumber, currentMode).catch((error: unknown) => {
                console.error('Error while changing GIF:', error);
              });
              await timer(3500);
              turnOff(carNumber).catch((error: unknown) => {
                console.error('Error while turning Off GIF:', error);
              });
            } else {
...
then turn off the racing number gif, then turn off the black and white gif. */
      if (/black and white/i.test(recentMessage.Message)) {
        isGifPlaying = true;
        changeGif('blackandwhite', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(3500);
        const recentRacingNumber = recentMessage.RacingNumber;
        if (recentRacingNumber) {
          changeGif(recentRacingNumber, currentMode, recentRacingNumber).catch((error: unknown) => {
            console.error('Error while changing GIF:', error);
          });
          await timer(3500);
          turnOff(recentRacingNumber, recentRacingNumber).catch((error: unknown) => {
            console.error('Error while turning off GIF:', error);
          });
          isGifPlaying = false;
        } else {
          if (debugOn) console.log(`No racing number GIF found for ${recentRacingNumber}`);
          turnOff('blackandwhite').catch((error: unknown) => {
            console.error('Error while turning off GIF:', error);
          });
          isGifPlaying = false;
        }
...
off after a certain amount of time. */
        ...
        if (carNumberMatch) {
          ...
          if (carNumber) {
            ...
            if (/5 second time penalty/i.test(recentMessage.Message)) {
              isGifPlaying = true;
              changeGif('timepenalty5sec', currentMode).catch((error: unknown) => {
                console.error('Error while changing GIF:', error);
              });
              await timer(3500);
              changeGif(carNumber, currentMode, carNumber).catch((error: unknown) => {
                console.error('Error while changing GIF:', error);
              });
              await timer(3500);
              turnOff(carNumber, carNumber).catch((error: unknown) => {
                console.error('Error while turning off GIF:', error);
              });
              isGifPlaying = false;
            }
            if (/10 second time penalty/i.test(recentMessage.Message)) {
              isGifPlaying = true;
              changeGif('timepenalty10sec', currentMode).catch((error: unknown) => {
                console.error('Error while changing GIF:', error);
              });
              await timer(3500);
              changeGif(carNumber, currentMode, carNumber).catch((error: unknown) => {
        ...
        if (carNumberMatch) {
          ...
          if (carNumber) {
            ...
            if (/10 second stop\/go penalty/i.test(recentMessage.Message)) {
              isGifPlaying = true;
              changeGif('stopgopenalty10sec', currentMode).catch((error: unknown) => {
                console.error('Error while changing GIF:', error);
              });
              await timer(3500);
              changeGif(carNumber, currentMode, carNumber).catch((error: unknown) => {
      ...
      if (
        ...
        isGifPlaying = true;
        changeGif('rs', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(20000);
        turnOff('rs').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      if (/standing start procedure/i.test(recentMessage.Message) && recentMessage.SubCategory !== 'SessionResume') {
        isGifPlaying = true;
        changeGif('ss', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(20000);
        turnOff('ss').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      if (/drs enabled/i.test(recentMessage.Message) && extraFlagSwitch) {
        isGifPlaying = true;
        changeGif('DRSenabled', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(3500);
        turnOff('DRSenabled').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      if (/drs disabled/i.test(recentMessage.Message) && extraFlagSwitch) {
        isGifPlaying = true;
        changeGif('DRSdisabled', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(3500);
        turnOff('DRSdisabled').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      if (/pit lane entry closed/i.test(recentMessage.Message)) {
        isGifPlaying = true;
        changeGif('pitclosed', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(3500);
        turnOff('pitclosed').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      if (/pit exit open/i.test(recentMessage.Message)) {
        isGifPlaying = true;
        changeGif('green', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(3500);
        turnOff('green').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      ...
      if (/recovery vehicle on track/i.test(recentMessage.Message) && extraFlagSwitch) {
        isGifPlaying = true;
        changeGif('recoveryvehicle', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(5000);
        turnOff('recoveryvehicle').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      if (/medical car deployed/i.test(recentMessage.Message) && extraFlagSwitch) {
        isGifPlaying = true;
        changeGif('medicalcar', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(5000);
        turnOff('medicalcar').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      if (/aborted start/i.test(recentMessage.Message) && extraFlagSwitch) {
        isGifPlaying = true;
        changeGif('abortedstart', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(5000);
        turnOff('abortedstart').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      ...
      if (recentMessage.SubCategory === 'TrackSurfaceSlippery') {
        isGifPlaying = true;
        changeGif('slippery', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(5000);
        turnOff('slippery').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }

      if (/double yellow/i.test(recentMessage.Message) && !/lap deleted/i.test(recentMessage.Message) && !sc && !vsc) {
        isGifPlaying = true;
        changeGif('dyellow', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(10000);
        turnOff('dyellow').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }
      ...
      if (/blue flag/i.test(recentMessage.Message) && recentMessage.Flag !== 'CHEQUERED' && blueFlagSwitch) {
        isGifPlaying = true;
        changeGif('blue', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(3500);
        /* Using a regular expression to match the message to a pattern. */
        ...
        if (recentRacingNumber) {
          ...
          if (debugOn) console.log(`No racing number GIF found for ${recentRacingNumber}`);
          turnOff('blue').catch((error: unknown) => {
            console.error('Error while turning off GIF:', error);
          });
          isGifPlaying = false;
        }
      ...
      if (
        LT_Data.SessionInfo.Type === 'Race' &&
        LT_Data.LapCount.CurrentLap === LT_Data.LapCount.TotalLaps &&
        LT_Data.SessionStatus.Status !== 'Finished' &&
        !sc &&
        !vsc &&
        !hasPlayedFinalLap
      ) {
        isGifPlaying = true;
        hasPlayedFinalLap = true;
        changeGif('finallap', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(8000);
        turnOff('finallap').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
      }

      if (/chequered flag/i.test(recentMessage.Message) && LT_Data.SessionStatus.Status === 'Finished') {
        isGifPlaying = true;
        changeGif('chequered', currentMode).catch((error: unknown) => {
          console.error('Error while changing GIF:', error);
        });
        await timer(90000);
        turnOff('chequered').catch((error: unknown) => {
          console.error('Error while turning off GIF:', error);
        });
        isGifPlaying = false;
        hasPlayedFinalLap = false;
      }
    }
  }
};
    ...
    if (recentTrackStatus !== undefined && recentTrackStatus !== currentTrackStatus) {
      ...
      switch (recentTrackStatus) {
        ...
        case '2': {
          ...
          red = false;
          isGifPlaying = true;
          changeGif('yellow', currentMode).catch((error: unknown) => {
            console.error('Error while changing GIF:', error);
          });
          break;
        }
        case '4': {
          ...
          red = false;
          isGifPlaying = true;
          changeGif('sc', currentMode).catch((error: unknown) => {
            console.error('Error while changing GIF:', error);
          });
          break;
        }
        case '5': {
          ...
          red = true;
          isGifPlaying = true;
          changeGif('red', currentMode).catch((error: unknown) => {
            console.error('Error while changing GIF:', error);
          });
          break;
        }
        case '6': {
          ...
          red = false;
          isGifPlaying = true;
          changeGif('vsc', currentMode).catch((error: unknown) => {
            console.error('Error while changing GIF:', error);
          });
          break;
        }
      ...
      switch (Rain) {
        ...
        case '1': {
          ...
          if (!lightOnRain && !pixoostartup) {
            ...
            changeGif('rain', currentMode).catch((error: unknown) => {
              console.error('Error while changing GIF:', error);
            });
            lightOnRain = true;
          }
...
value. */
  ...
  $('#alwaysOnTopSwitch').on('change', () => {
    void globalThis.api.setAlwaysOnTop();
  });
} else {
  $('#alwaysOnTop').remove();
  $('#networkSettings').remove();
