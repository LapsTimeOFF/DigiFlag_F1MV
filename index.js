		...
		if (sc === true) {
			...
			if (currentMode === 1) {
				const res = await httpGet(
					`http://localhost:9093/pixoo/void/${currentTheme}/${pixooIP}`
				);
				if (res !== "OK") {
 ...
 * </code>
 * @param flag - the flag to change to
 * @param mode - 1 = Pixoo64, 2 = DigiFlag
 * @returns a Promise.
 */
async function changeGif(flag, mode) {
	...
	if (mode === 1 && currentMode === 1) {
		const res = await httpGet(
			`http://localhost:9093/pixoo/${flag}/${currentTheme}/${pixooIP}`
		);
		if (res !== "OK") {
			console.log("Failed to change GIF on Pixoo64");
		}
